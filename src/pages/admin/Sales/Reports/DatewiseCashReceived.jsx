...
const DatewiseCashReceived = () => {
  ...
  const API_URL = `${import.meta.env.VITE_API_BASE_URL}/cash-received`;
  ...
  useEffect(() => {
    ...
    fetchSalesmen();
    fetchCashRecords();
  }, [fetchCustomers, fetchSalesmen, fetchCashRecords]);
  ...
  const handleSave = async () => {
    ...
    const data = { date, salesman, customer, amount, paymentMethod, remarks };
    const headers = {
      ...
      .then(async (result) => {
        if (result.isConfirmed) {
          try {
            ...
            setCashRecords(cashRecords.filter((r) => r._id !== id));
            swal.fire(
              "Deleted!",
              "Cash record deleted successfully.",
              "success"
            );
          } catch {
            swal.fire("Error!", "Failed to delete cash record.", "error");
          }
        }
      });
  };
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-newPrimary">
            Datewise Cash Received
          </h1>
          <p className="text-gray-500 text-sm">
            Manage cash received on specific dates
          </p>
        </div>
        <button
      ...
      <div className="rounded-xl border border-gray-200 overflow-hidden">
        <div className="overflow-x-auto">
          <div className="min-w-[900px]">
            <div className="hidden lg:grid grid-cols-[50px_1fr_1fr_1fr_1fr_1fr_120px] gap-6 bg-gray-100 py-3 px-6 text-xs font-semibold text-gray-600 uppercase border-b border-gray-200">
              <div>SR#</div>
              <div>Date</div>
              <div>Salesman</div>
              <div>Customer</div>
            ...
            <div className="flex flex-col divide-y divide-gray-100 max-h-[400px] overflow-y-auto">
              {loading ? (
                ...
                cashRecords.map((r, index) => (
                  <div
                    key={r._id}
                    className="hidden lg:grid grid-cols-[50px_1fr_1fr_1fr_1fr_1fr_120px] items-center gap-6 px-6 py-4 text-sm bg-white hover:bg-gray-50 transition"
                  >
                    <div>{index + 1}</div>
                    <div>{r.date}</div>
          ...
          >
            <div className="flex justify-between items-center p-4 border-b sticky top-0 bg-white rounded-t-2xl">
              <h2 className="text-xl font-bold text-newPrimary">
                {isEdit ? "Update Cash Record" : "Add New Cash Record"}
              </h2>
              <button
...
export default DatewiseCashReceived;
